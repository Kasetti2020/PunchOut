({
    ...
    FetchRetailerDocnotesAndAttach:function(component, event, helper,selectedRetailer)
    {
        var action = component.get("c.getDocCustomNotesAttachRetailer");
        action.setParams({retailerId : JSON.stringify(selectedRetailer) });
        action.setCallback(this, function(response) {
            var state = response.getState();
            //alert('Fetch>>>>>'+state);
            if (state === "SUCCESS") {
                ...
                if(storeResponse !=0){
                //alert(JSON.stringify(storeResponse));
                component.set("v.FileNotFoundforRetailer",false);
                component.set("v.FilesFound",true);
                component.set("v.ListofFilesRetailer",storeResponse);
                } else{
                    component.set("v.FilesFound",false);
                    component.set("v.FileNotFoundforRetailer",true);
                    var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        title: $A.get("$Label.c.Warning"),
                        message: $A.get("$Label.c.File_not_Found_for_the_Retailer"),
                        type: "warning"
                    });
                    toastEvent.fire();
                    //alert("File not Found for the Retailer.");
                   // $A.get('e.force:refreshView').fire();
                }
            }
        });
    ...
    FetchRelatedRetailer : function(component,event,helper){
        ...
        action.setCallback(this, function(actionResult) {
            ...
            if(status=="SUCCESS")
            {
            component.set('v.RetailerList',actionResult.getReturnValue());
            console.log(component.get('v.RetailerList'));
            }
        });
        $A.enqueueAction(action);
    }
