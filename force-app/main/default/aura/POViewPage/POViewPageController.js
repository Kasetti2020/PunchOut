({
    doint : function(component, event, helper,page) 
    {
        ...
        helper.getCartInfo(component, event, helper);
        const options = [
        ...
        */
        ...
        for (var i = 0; i < getCheckAllId.length; i++) 
        {
            if(getCheckAllId[i].IsSelected == true)
            {
                ...
                for(var j = 0; j < poli.length; j++) 
                {
                    if(poli[j].Supply_Quantity__c == 0)
                    {
                        ...
                        var toastEvent = $A.get("e.force:showToast");
                			 ...
                			 "type" : "error",
                			 "message": $A.get("$Label.c.Enter_0_if_Line_Item_not_required")
             				});
            				 toastEvent.fire();
                        return;
                    }
                ...
                if(POLIQtyArr.length>0 && (getCheckAllId[i].POLIwrap.length == POLIQtyArr.length))
                {
            ...
            if(getCheckAllId[i].IsSelected == true)
            {
                ...
                if(poli.length==0 || getCheckAllId[i].isAllqtyZero)	//validation to check if the selected PO have line items or not
                {
                    ...
                    toastEvent.setParams({
                        ...
                        message: $A.get("$Label.c.Please_check")  +getCheckAllId[i].POwrap.Order_Number__c+$A.get("$Label.c.Either_it_does_not_have_any_line_items_or_the_PO_has_been_cancelled"),
                        type: "warning"
                    });
                    toastEvent.fire();
                    return;
    ...
    //Test new component method
    viewSelectedPO : function(component, event, helper) {
    	
        //console.log("Checkedornot>>>>>"+component.get("v.DisableViewPO"));
        if(component.get("v.DisableViewPO")==true){
            
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
            title : $A.get("$Label.c.Warning"),
            message: $A.get("$Label.c.No_PO_is_added"),
            duration:' 5000',
            key: 'info_alt',
            type: 'warning',
            mode: 'pester'
    ...
    addPOToChild: function(component, event, helper){
        var selctedRecTemp = [];
        var temp = [];
        var getCheckAllId = component.get("v.POList.POLIList");
        //console.log('Polist>>>'+JSON.stringify(getCheckAllId));
        ...
        component.set("v.StorePOListIds",temp);
        //console.log('Added in list temp>>>>'+temp);
        console.log("StorePOListIds"+component.get("v.StorePOListIds"));
        if(component.get("v.StorePOListIds")==null || component.get("v.StorePOListIds")== "" || component.get("v.StorePOListIds")==undefined){
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                title : $A.get("$Label.c.Warning"),
                message:$A.get("$Label.c.Please_select_the_PO_and_add_it_to_cart"),
                duration:' 5000',
                type: 'Warning'
            });
            toastEvent.fire();      
            ...
            message: $A.get("$Label.c.Selected_PO_added_successfully"),
            duration:' 5000',
            key: 'info_alt',
            type: 'success',
            mode: 'pester'
        ...
        var action = component.get("c.storingPoCheckbox");
    ...
    getCancelledPOLI : function(component, event, helper) {
    	
       
            component.set("v.CancelledPOList",true);
        component.set("v.MainFlag",false);
    }
})