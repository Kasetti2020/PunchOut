({
    ...
    addWarehouse : function(component, event, helper) {
    ...
    addUser : function(component, event, helper) {
        ...
        if(retailer == null || retailer == ""){
            var text = 'Please select Warehouse';
            helper.selectErrorToast(component,event,helper,text);
            return;
        }
        //component.set("v.selectedRetailerRecord.Name",retailer);
        
       // alert("warehouse:"+retailer);
        var actionRetailer = component.get("c.fetchWarehouseId"); 
        ...
        actionRetailer.setCallback(this,function (response) {
            ...
            if (state === "SUCCESS") {                
                ...
                for (var i = 0; i < result.length; i++) {
                   // alert(result[i].Warehouse_name_holder__c);
                   // alert(result[i].Id);
                    component.set("v.selectedRetailerRecord.Warehouse__c",result[i].Warehouse_name_holder__c);
    ...
    deleteRetailer : function(component, event, helper) {
        var retailer = component.get("v.SearchKeyWord");        
        if(retailer == null || retailer == ""){
            var text = 'Please select Warehouse to delete';
            helper.selectErrorToast(component,event,helper,text);
            return;
        }
        ...
        action.setCallback(this,function (response) {
            ...
            if (state === "SUCCESS") {                
                ...
                if(status){
                    ...
                    var text = 'Warehouse deleted successfully.';
    ...
    Save: function(component, event, helper) {
        ...
        if(Object.keys(retailer).length === 0){
            document.getElementById('errorMissingSelected').innerHTML='Please Select Warehouse';
            return;
        }
        ...
        action.setCallback(this,function (response) {
            ...
            if (state === "SUCCESS") {                
                ...
                if(status){
                    ...
                    //component.set("v.selectedUserRecord","");
                    var text = 'Warehouse saved successfully.';
                    helper.successToast(component,event,helper,text);
                }
            }else if(state === "ERROR") {
                var errors = response.getError();
